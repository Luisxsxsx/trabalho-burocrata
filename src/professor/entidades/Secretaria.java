...
public class Secretaria {
    ...
    protected void despachar(Processo processo, Burocrata burocrata) {
        ...
        for (Documento doc : copiaDoProcesso) {
            if (doc.getCodigoCurso().equals(CodigoCurso.POS_GRADUACAO_ENGENHARIA)
                    || doc.getCodigoCurso().equals(CodigoCurso.POS_GRADUACAO_ENGENHARIA_ELETRICA)
                    || doc.getCodigoCurso().equals(CodigoCurso.POS_GRADUACAO_ENGENHARIA_SOFTWARE)) {
                posgraduacao = true;
            } else {
                graduacao = true;
            }
        ...
        if (contagemDeOficiosECirculares > 0) {
            ...
            for (int ocorrencias : destinatarios.values()) {
                // >= é mais seguro por causa de possíveis repetições de destinatários no mesmo
                // documento
